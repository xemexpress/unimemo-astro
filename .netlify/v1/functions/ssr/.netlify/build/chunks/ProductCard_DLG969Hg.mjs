...
import 'kleur/colors';
import { P as ProductUtils, S as StringUtils } from './coreUtils_CRoQrX7v.mjs';
...
const $$ProductCard = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<article class="group relative transition-all duration-300 ease-in-out overflow-hidden rounded-lg"${addAttribute(sku, "id")}> <a${addAttribute(`${ROUTES.PRODUCTS(slug)}?p=${currentPage}${productT.length > 0 ? `&product-type=${productT}` : ""}${platingT.length > 0 ? `&plating-type=${platingT}` : ""}${metalT.length > 0 ? `&metal-type=${metalT}` : ""}`, "href")} class="block relative"> ${renderComponent($$result, "Image", $$Image, { "class": "rounded-lg aspect-square transition-transform duration-300 ease-in-out group-hover:scale-105 group-hover:shadow-md", "src": images[0], "alt": `${sku} ${name}`, "width": 400, "height": 400, "loading": "eager", "fetchpriority": "high", "decoding": "async" })} <div class="absolute top-2 left-2 bg-white bg-opacity-75 px-2 py-1 rounded text-sky-900 capitalize text-xs max-sm:text-[11px]"> ${productType} </div> <div class="max-sm:hidden absolute top-2 right-2 bg-white bg-opacity-75 px-2 py-1 rounded text-xs max-sm:text-[11px]"> ${sku} </div> <div class="hidden absolute top-2 left-2 right-2 bg-white bg-opacity-75 px-2 py-1 justify-between items-center text-xs max-sm:text-[11px] rounded text-sky-900 capitalize max-sm:flex"> <span> ${productType} </span> <span> ${sku} </span> </div> <div class="absolute bottom-2 left-2 bg-white bg-opacity-75 px-2 py-1 rounded text-xs max-sm:text-[11px]">
